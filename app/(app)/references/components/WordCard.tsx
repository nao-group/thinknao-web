...
export function WordCard({ entry, onClick }: { entry: WordEntry; onClick: () => void }) {
  return (
    ...
    >
      ...
      <Text size="sm" c="var(--app-text-muted)" lh={1.55} style={{ flex: 1 }}>
        {entry.definition.length > 90 ? entry.definition.slice(0, 90) + "…" : entry.definition}
      </Text>
      {entry.example && (
        <Box className="reference-card__accent" px="xs" py={4} style={{ backgroundColor: SURFACE, borderRadius: rem(6) }}>
          <Text size="xs" c="var(--app-text-muted)" style={{ fontStyle: "italic" }}>
            e.g. {entry.example}
          </Text>
        </Box>
      )}
